...
<style>
    .container{
        ...
        background: linear-gradient(180deg, rgba(32,40,51,1) 2%, rgba(69,89,119,1) 50%,#2a3443 89%);
    ...
    .profile img {
        ...
        height:100px;
        border-radius: 999px;
        border:5px solid #1eb278;
        outline: 2px solid #8d4551;
        margin-bottom:20px;
        margin-top: 30px
    }
    .contactsBar div{
...
<script>
    window.onload = function ()
    {
        ...
        for (let i=0;i<text.length;i++) {
            setTimeout(()=>{
                if(keywords.find(e=>e[0]===i||e[1]>i && e[0]<i)) return name.innerHTML += `<span>${text[i]}</span>`
